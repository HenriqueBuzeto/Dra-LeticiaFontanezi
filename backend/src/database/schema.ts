...
/** Usuário: login, perfil completo, notificações */
export const user = pgTable('user', {
  ...
  genero: text('genero'), // nao_informar | feminino | masculino | outro
  ...
  preferenciaLembrete: text('preferencia_lembrete'), // push | email | whatsapp
...
export const appointment = pgTable('appointment', {
  ...
  checkinStatus: text('checkin_status'), // vai_comparecer | nao_comparecer
...
/** Gamificação: log de ações que geram pontos */
export const pointLog = pgTable('point_log', {
  id: text('id').primaryKey(),
  userId: text('user_id').notNull().references(() => user.id, { onDelete: 'cascade' }),
  action: text('action').notNull(), // escovacao | fio_dental | consulta_presente | limpeza_bucal | uso_enxaguante | checkin_semanal
  points: integer('points').notNull(),
  metadata: text('metadata'),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
})

/** Recompensas (brindes por pontos) – admin gerencia */
export const rewardItem = pgTable('reward_item', {
  id: text('id').primaryKey(),
  name: text('name').notNull(),
  pointsRequired: integer('points_required').notNull(),
  type: text('type').notNull(), // escova | kit | consulta | brinde
